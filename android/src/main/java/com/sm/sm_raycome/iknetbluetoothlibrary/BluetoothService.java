...
import com.sm.sm_raycome.iknetbluetoothlibrary.util.ClsUtils;
...
//						remoteDevice.setPin(pinStrSingleMode.getBytes());
	...
	private void postBTMsg(int flag, int sflag, byte[] data) {
        // Debug Log: Print all incoming flags to understand what the device is sending
        Log.i(TAG, "postBTMsg: flag=" + flag + " sflag=" + sflag + " dataLen=" + (data!=null?data.length:0));
		if(connectedDeviceType == DeviceType.TYPE_9000){
			...
			if (flag == 1) {
				...
				if (sflag == 6) {
                    ...
                    Log.i(TAG, "Measurement Data Packet Received (Length: " + data.length + ")");
					// Handle both 2-byte (TYPE_9000 style) and 6-byte (TYPE_88A style) packets
					int running = 0;
					if (data.length >= 6) {
						// Standard TYPE_88A format: 6+ bytes
						final int highflag = data[1] & 0xFF;
						final int lowflag = highflag ^ (data[4] & 0xFF);
						running = (highflag << 8) + lowflag;
					} else if (data.length >= 2) {
						// TYPE_9000 style format: 2 bytes (high byte, low byte)
						running = ((data[0] & 0xFF) << 8) + (data[1] & 0xFF);
					} else {
						Log.w(TAG, "Data length too short for measurement packet!");
						return;
					}
                    Log.i(TAG, "Broadcasting Running Pressure: " + running);
